...
ffmpeg -nostats -i $OUTPUT -filter_complex "[0:a]silencedetect=n=-50dB:d=1[outa]" -map [outa] -f s16le -y /dev/null &> $SILENCE_RAW &&
...
INIT { $ss=0; $se=0; }
  if (/silence_start: (\S+)/) { $ss=$1; $ctr+=1; printf "ffmpeg -nostdin -i $ENV{FULL_TRACK} -ss %f -t %f $ENV{F} -y b_%03d.ts\n", $se, ($ss-$se), $ctr; }
